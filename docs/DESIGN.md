...
- **Screen content scrolls.** A screen whose content can grow — a profile
  ...
  (LODGY-96). The same audit, run against every screen rather than the one
  that had already been caught, found 17 more with the identical unscrolled
  `Column` and a further one — PIN setup — whose keypad sat in a
  `Box` overlaid on the content instead of a plain `Column`, which on a
  short screen could clip or overlap the content above it rather than
  merely leave it unreachable; that one was restructured into a scrollable
  content region with the keypad as a fixed footer below it, not just given
  the same modifier (LODGY-97).
